...
const OUTPUT_DIR = join(dirname(fileURLToPath(import.meta.url)), '..', 'wiki-data', 'pages')
const BATCH_SIZE = 50
const DELAY_MS = 500
const MAX_PAGES = process.env.MAX_PAGES ? parseInt(process.env.MAX_PAGES) : Infinity
...
async function getAllPageTitles(max: number): Promise<string[]> {
  ...
  do {
    ...
    const data = await fetchJson(`${WIKI_API}?${params}`) as Record<string, unknown>
...
async function main() {
  ...
  for (let i = 0; i < capped.length; i += BATCH_SIZE) {
    ...
    for (const title of batch) {
      const exists = await fileExists(join(OUTPUT_DIR, `${safeName(title)}.json`))
      if (exists) {
        skipped++
      } else {
        toFetch.push(title)
    ...
    console.log(`${done}/${total} — ${downloaded} downloaded, ${skipped} skipped`)
  }
